...
*/
            ...
            while (true) {
                game.display ();
                if (game.getMove () == false) {
                    gameDAO.updateWholeGame (game);
                    // updateplayer stats
                    break;
                }
                else
                {
                    // This needs to be improved should not update every thing just what has changed but running out of time
                    gameDAO.updateWholeGame (game);
                }
            }

            // Update results etcetera
