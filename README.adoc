...
We took a WORKING spring boot project and completely changed it bit by bit to meet our projects needs. Trying to create a spring boot project from scratch proved very difficult and frustrating. As supposedly working projects often failed. Usually due to the version of spring boot.

The scrabble game is state driven

.Gamestate
|===
|Player 1 |Player 2 |Comment 3

|Creates a game
|Joins the game
|The game now randomly chooses who plays first

|
|
|

|Plays letters to form Word(s)
|Accepts Word(s)
|Player 1's score is adjusted, new letters are dealt and Player 2 is to move next

|
|Challenges Word(s) unsuccessfully
|Player 1's score is adjusted, new letters are dealt and Player 2 is to move next

|
|Challenges Word(s) successfully
|Player 1 has to try again

|Plays letters to form different Word(s)
|Accepts Word(s)
|Player 1's score is adjusted, new letters are dealt and Player 2 is to move next

|
|Challenges Word(s) unsuccessfully
|Player 1's score is adjusted, new letters are dealt and Player 1 is to move next. Player 2 misses a turn.

|
|Challenges Word(s) successfully
|Player 1's letters are returned. Player 1 misses this turn. Player 2 is to move next.

|Exchange letters
|
|Instead of playing letters on the board. A player may exchange 1 to 7 letters with the letter bag, providing there are enough letters. The letter exchange is done all at once. Player 2 moves next.

|Pass
|
| Three passes in a row and the Game is declared a draw.
...
communicate with our server through STOMP over websocket. We also import `rack.js`, which
...
The `app.css` can be omitted if you like, or you can create an empty
